...
def parse_inflearn_course(html_content, course_title):
    soup = BeautifulSoup(html_content, "html.parser")

    results = []
    section_titles = soup.find_all(
        "p", class_="mantine-Text-root css-542wex mantine-5jtosh"
    )
    ...
    for element in soup.find_all():
        if (
            ...
            and "mantine-Text-root"
            and "css-542wex"
            and "mantine-5jtosh" in element.get("class")
        ):
            ...
            and "mantine-Text-root"
            and "css-1av9he0"
            and "mantine-105cbrz" in element.get("class")
        ):
            if current_section:
        ...
        if (
            ...
            and "mantine-Text-root"
            and "css-48jxi2"
            and "mantine-1q4x896" in element.get("class")
        ):
            if current_section:
...
def save_to_excel(df, filename="inflearn_course_analysis.xlsx"):
    """
    ...
    # 데이터프레임을 Excel에 저장
    df.to_excel(writer, index=False, sheet_name="Inflearn Course")

    # 워크시트 가져오기
    workbook = writer.book
    worksheet = writer.sheets["Inflearn Course"]
...
def main():
    # 사용 예시
    course_title = input("Inflearn 강의 제목을 입력하세요: ")
    ...
    if option == "1":
        ...
        url = input("Inflearn 강의 커리큘럼 URL을 입력하세요: ")
        headers = {
    ...
    # 데이터 파싱
    df = parse_inflearn_course(html_content, course_title)
